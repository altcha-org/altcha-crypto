...
import { DecipherGCM, createCipheriv, createDecipheriv } from 'node:crypto';
import { Readable, Writable } from 'node:stream';
import { pipeline } from 'node:stream/promises';
import aes from './aes.ts';
import rsa from './rsa.ts';
import { START_BYTES, AES_IV_LEN, AES_KEY_LEN, readHead } from './cipher.ts';
import { compareByteArrays } from './helpers.ts';
...
export async function decryptStream(
  ...
  return new Promise((resolve, reject) => {
    ...
    input.on('data', (chunk: Buffer) => {
      if (!decipher) {
        const { encKey, iv, startBytes, tail } = readHead(
          new Uint8Array(
            chunk.buffer.slice(
              chunk.byteOffset,
